...
class WidgetPickTest {
    ...
    private var n = 0L
    private fun timed(title: String, b: Long, e: Long) = Occurrence(++n, 1, title, b, e, false, null)
    private fun allDay(title: String, from: LocalDate, days: Int = 1) = Occurrence(++n, 1, title, midnight(from), midnight(from.plusDays(days.toLong())), true, null)
    private fun pick(items: List<Occurrence>, now: Long) = WidgetPick.line(items, now)?.title
    ...
    @Test fun nextChangeBoundaries() {
        assertEquals(at(9), WidgetPick.nextChange(items, at(8)))
        assertEquals(at(10), WidgetPick.nextChange(items, at(9, 10)))
        assertEquals(at(11), WidgetPick.nextChange(items, at(10, 30)))
        assertEquals(at(13), WidgetPick.nextChange(items, at(11, 30)))
        assertEquals(midnight(day.plusDays(1)), WidgetPick.nextChange(listOf(holiday), at(16)))
    }
}
